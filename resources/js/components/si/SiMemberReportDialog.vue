...
watch(
    ...
    (member) => {
        if (member) {
            spiritualValues.value = member.spiritual_assessments.map(
                (a) => a.value,
            );
            activityValues.value = member.activity_assessments.map(
                (a) => a.value,
            );
        }
    },
...
function saveAssessments() {
    ...
    router.patch(
        ...
        {
            spiritual_assessments: spiritualValues.value,
            activity_assessments: activityValues.value,
        },
        {
            preserveScroll: true,
            onSuccess: () => {
...
const chartData = computed(() => {
    ...
    const data = [
        ...props.categories.map(
            (c) =>
                +(
                    (props.member!.category_scores[c.id] ?? 0) *
                    c.weight *
                    100
                ).toFixed(2),
        ),
...
<template>
    <Dialog :open="open" @update:open="emit('update:open', $event)">
        <DialogContent
            class="flex max-h-[90vh] w-full flex-col overflow-hidden sm:!max-w-5xl"
        >
            <DialogHeader>
            ...
            <template v-if="member">
                <div class="flex-1 overflow-y-auto pr-1">
                    <!-- Star rating + overall -->
                    <div
                        class="flex items-center justify-between rounded-xl bg-muted/40 px-6 py-4"
                    >
                        <div>
                            <div
                    ...
                    <div class="mt-4 space-y-4 border-t pt-4">
                        <div>
                            <label
                                ...
                                >Spiritual Assessment</label
                            >
                            <Multiselect
                                v-model="spiritualValues"
                                :options="
                                    spiritualAssessmentOptions.map((o) => ({
                                        value: o.value,
                                        label: o.label,
                                    }))
                                "
                        ...
                        <div>
                            <label
                                class="mb-1.5 block text-sm font-medium text-muted-foreground"
                                >Activity Assessment</label
                            >
                            <Multiselect
                                v-model="activityValues"
                                :options="
                                    activityAssessmentOptions.map((o) => ({
                                        value: o.value,
                                        label: o.label,
                                    }))
                                "
